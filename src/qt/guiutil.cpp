...
{
    // Convert hpxt:// to hpxt:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("hpxt://"))
    {
        uri.replace(0, 12, "hpxt:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "HPXT.lnk";
}
...
{
    return GetAutostartDir() / "hpxt.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("HPXT-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  hpxt-qt [" + tr("command-line options") + "]                     " + "\n";
